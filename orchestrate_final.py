...
# ---------- main ----------
def main():
    ap = argparse.ArgumentParser(description="LLM + Jotai orchestrator with san/sanint (+AFL), resume-aware LLM gen")
    ...
    # 1) LLM generation (B1..B4) — resume-aware with 429 backoff
    ...
    for fam in ("B1", "B2", "B3", "B4"):
        sources += list((d_gen / fam).rglob("*.c"))
