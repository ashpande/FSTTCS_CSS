...
def parse_percentage(rate_str):
    ...
    try:
        ...
        return float(str(rate_str).strip('%'))
    except ValueError:
...
df_sorted = df.sort_values('Rate_Numeric', ascending=False).reset_index(drop=True) # Reset index after sorting
...
ax.set_ylim(bottom=-1, top=max(df_sorted['Rate_Numeric']) * 1.15) # Set ylim slightly above max, start slightly below 0
...
# Iterate with index to correctly fetch the label from the sorted dataframe
for i, bar in enumerate(bars):
    yval = bar.get_height()
    # Get the original string label from the sorted DataFrame using the index
    label_text = df_sorted['Rate (vs Built) %'].iloc[i]
    ...
    plt.text(bar.get_x() + bar.get_width()/2.0, # Use float division for center
             yval + v_offset,
             label_text,
